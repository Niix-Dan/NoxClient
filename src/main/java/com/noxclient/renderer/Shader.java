...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            NoxClient.LOG.error("Failed to compile vertex shader (" + vertPath + "): " + vertError);
        ...
        if (fragError != null) {
            NoxClient.LOG.error("Failed to compile fragment shader (" + fragPath + "): " + fragError);
        ...
        if (programError != null) {
            NoxClient.LOG.error("Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(NoxClient.mc.getResourceManager().getResource(new MeteorIdentifier("shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
