...
public class WBlockPosEdit extends WHorizontalList {
    ...
    public void init() {
        ...
        if (Utils.canUpdate()) {
            ...
            click.action = () -> {
                ...
                previousScreen = NoxClient.mc.currentScreen;
                NoxClient.mc.setScreen(null);
            };

            WButton here = add(theme.button("Set Here")).expandX().widget();
            here.action = () -> {
                lastValue = value;
                set(new BlockPos(NoxClient.mc.player.getBlockPos()));
                newValueCheck();
    ...
    private void onStartBreakingBlock(StartBreakingBlockEvent event) {
        if (clicking) {
            ...
            NoxClient.mc.setScreen(previousScreen);
        }
    }
