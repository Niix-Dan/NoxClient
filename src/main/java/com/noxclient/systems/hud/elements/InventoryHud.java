...
public class InventoryHud extends HudElement {
    ...
    public void render(HudRenderer renderer) {
        ...
        if (NoxClient.mc.player != null) {
            for (int row = 0; row < 3; row++) {
                for (int i = 0; i < 9; i++) {
                    int index = row * 9 + i;
                    ItemStack stack = hasContainer ? containerItems[index] : NoxClient.mc.player.getInventory().getStack(index + 9);
    ...
    private ItemStack getContainer() {
        ...
        ItemStack stack = NoxClient.mc.player.getOffHandStack();
        if (Utils.hasItems(stack) || stack.getItem() == Items.ENDER_CHEST) return stack;

        stack = NoxClient.mc.player.getMainHandStack();
        if (Utils.hasItems(stack) || stack.getItem() == Items.ENDER_CHEST) return stack;
