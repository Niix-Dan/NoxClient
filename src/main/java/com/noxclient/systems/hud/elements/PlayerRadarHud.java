...
import com.noxclient.systems.hud.*;
...
public class PlayerRadarHud extends HudElement {
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (PlayerEntity entity : getPlayers()) {
            if (entity.equals(NoxClient.mc.player)) continue;
            if (!friends.get() && Friends.get().isFriend(entity)) continue;

            String text = entity.getEntityName();
            if (distance.get()) text += String.format("(%sm)", Math.round(NoxClient.mc.getCameraEntity().distanceTo(entity)));
    ...
    public void render(HudRenderer renderer) {
        ...
        for (PlayerEntity entity : getPlayers()) {
            ...
            if (distance.get()) {
                distanceText = String.format("(%sm)", Math.round(NoxClient.mc.getCameraEntity().distanceTo(entity)));
    ...
    private List<AbstractClientPlayerEntity> getPlayers() {
        players.clear();
        players.addAll(NoxClient.mc.world.getPlayers());
        if (players.size() > limit.get()) players.subList(limit.get() - 1, players.size() - 1).clear();
        players.sort(Comparator.comparingDouble(e -> e.distanceTo(NoxClient.mc.getCameraEntity())));
