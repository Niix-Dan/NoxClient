...
public class HoleHud extends HudElement {
    ...
    private Direction get(Facing dir) {
        ...
        return Direction.fromRotation(MathHelper.wrapDegrees(NoxClient.mc.player.getYaw() + dir.offset));
    }

    private void drawBlock(HudRenderer renderer, Direction dir, double x, double y) {
        Block block = dir == Direction.DOWN ? Blocks.OBSIDIAN : NoxClient.mc.world.getBlockState(NoxClient.mc.player.getBlockPos().offset(dir)).getBlock();
        ...
        ((WorldRendererAccessor) NoxClient.mc.worldRenderer).getBlockBreakingInfos().values().forEach(info -> {
            if (info.getPos().equals(NoxClient.mc.player.getBlockPos().offset(dir))) {
