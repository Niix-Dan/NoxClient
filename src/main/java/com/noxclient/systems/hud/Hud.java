...
import com.noxclient.systems.hud.elements.*;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    ...
    private void onRender(Render2DEvent event) {
        if (Utils.isLoading()) return;
        if (!(active && ((!NoxClient.mc.options.hudHidden && !NoxClient.mc.options.debugEnabled) || HudEditorScreen.isOpen()))) return;
