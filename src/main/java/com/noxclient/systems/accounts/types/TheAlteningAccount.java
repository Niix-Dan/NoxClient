...
import com.noxclient.systems.accounts.AccountUtils;
...
public class TheAlteningAccount extends Account<TheAlteningAccount> {
    ...
    public boolean login() {
        ...
        try {
            ...
            NoxClient.LOG.error("Failed to login with TheAltening.");
            return false;
        }
    }

    private YggdrasilUserAuthentication getAuth() {
        YggdrasilUserAuthentication auth = (YggdrasilUserAuthentication) new YggdrasilAuthenticationService(((MinecraftClientAccessor) NoxClient.mc).getProxy(), "", Environment.create(AUTH, ACCOUNT, SESSION, SERVICES, "The Altening")).createUserAuthentication(Agent.MINECRAFT);
