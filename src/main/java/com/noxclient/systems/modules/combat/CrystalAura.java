...
import com.noxclient.mixininterface.IBox;
...
import com.noxclient.events.entity.EntityRemovedEvent;
...
import com.noxclient.utils.world.BlockIterator;
...
public class CrystalAura extends Module {
    ...
    private final Setting<Boolean> anyplacement = sgPlace.add(new BoolSetting.Builder()
        .name("any-block-placement")
        .description("Try to place end crystals at any block.")
        .defaultValue(false)
        .build()
    );

    private final Setting<Boolean> airplacement = sgPlace.add(new BoolSetting.Builder()
        .name("air-placement")
        .description("Try to place end crystals in the air.")
        .defaultValue(false)
        .build()
    );
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            boolean hasBlock = anyplacement.get() || blockState.isOf(Blocks.BEDROCK) || blockState.isOf(Blocks.OBSIDIAN);
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        // Place
        if (supportBlock == null || airplacement.get()) {
            // Place crystal
