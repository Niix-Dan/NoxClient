...
public class NcpEFly extends Module {
    ...
    //Automatic
    private final Setting<Integer> startAt = sgAutomatic.add(new IntSetting.Builder().name("start-fly-at").description("The Y coordinate to start automatic module.").defaultValue(0).min(-59).sliderMin(-59).sliderMax(319).build());
    private final Setting<Integer> targetY = sgAutomatic.add(new IntSetting.Builder().name("target-y").description("The target coordinate for boosting.").defaultValue(0).min(-59).sliderMin(-59).sliderMax(319).build());
    ...
    public NcpEFly() {
        super(Categories.Dev, "NCP-elytra-fly", "Elytra Fly that works on ncp servers.");
    }
