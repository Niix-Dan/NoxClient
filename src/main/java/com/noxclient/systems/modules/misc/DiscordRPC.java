...
public class DiscordRPC extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("NOX ON TOP!!", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public DiscordRPC() {
        super(Categories.Misc, "discord-presence", "Displays Nox Client as your presence on Discord.");
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (Utils.canUpdate()) {
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails("Nox Client " + (NoxClient.DEV_BUILD.isEmpty() ? NoxClient.VERSION : NoxClient.VERSION + " " + NoxClient.DEV_BUILD));
