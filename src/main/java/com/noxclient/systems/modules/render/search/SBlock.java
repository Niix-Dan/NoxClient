...
package com.noxclient.systems.modules.render.search;
...
public class SBlock {
    ...
    public void update() {
        state = NoxClient.mc.world.getBlockState(blockPos.set(x, y, z));
        neighbours = 0;
    ...
    private boolean isNeighbour(Direction dir) {
        ...
        BlockState neighbourState = NoxClient.mc.world.getBlockState(blockPos);

        if (neighbourState.getBlock() != state.getBlock()) return false;

        VoxelShape cube = VoxelShapes.fullCube();
        VoxelShape shape = state.getOutlineShape(NoxClient.mc.world, blockPos);
        VoxelShape neighbourShape = neighbourState.getOutlineShape(NoxClient.mc.world, blockPos);
    ...
    private boolean isNeighbourDiagonal(double x, double y, double z) {
        ...
        return state.getBlock() == NoxClient.mc.world.getBlockState(blockPos).getBlock();
    }
