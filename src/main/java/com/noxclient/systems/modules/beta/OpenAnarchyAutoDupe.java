...
import com.noxclient.utils.player.ImmediateBlockIterator;
import com.noxclient.utils.player.InvUtils;
import com.noxclient.utils.render.RenderUtils;
...
public class OpenAnarchyAutoDupe extends Module {
    ...
    public OpenAnarchyAutoDupe() {
        super(Categories.Misc, "OpenAnarchyAutoDupe", "Auto dupe for openanarchy");
    }
    ...
    public void onDeactivate() {
        if (redstone != null && mc.world != null && mc.player != null && mc.world.isAir(redstone) && InvUtils.switchtoitem(Items.REDSTONE, true, true)) {
                    ...
                    if (pistons.stream().anyMatch(s -> entity.getBlockPos().equals(s.offset(mc.world.getBlockState(s).get(Properties.FACING))))) {
                        if (((ItemFrameEntity) entity).getHeldItemStack().isEmpty()) {
                            ...
                            if (duperedstonewhenno.get() && (InvUtils.find(Items.REDSTONE).count() <= redstonedupeamount.get() || (redstonedupe && InvUtils.find(Items.REDSTONE).count() <= 60))) {
                                redstonedupe = true;
                                if (InvUtils.switchtoitem(Items.REDSTONE, true, true)) {
                                ...
                                redstonedupe = false;
                                if (InvUtils.switchtoitem(InvUtils.find(stack -> itemtoplace.get().itemPredicate.test(stack.getItem())), true, true, itemtoplace.get().item)) {
                                    mc.interactionManager.interactEntity(mc.player, entity, Hand.MAIN_HAND);
    ...
    public void onRender(Render3DEvent event) {
        ...
        if (currentframe != null && (stage == Stage.Placeshulker || stage == Stage.Waitforshulker) && renderframes.get()) {
            ...
            if (direction != null) {
                RenderUtils.renderQuad(currentframe.getBlockPos(), direction.getOpposite(),event, new Color(1f,0f,0f,0.5f));
            }
        }
