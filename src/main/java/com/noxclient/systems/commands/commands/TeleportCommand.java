...
import com.noxclient.utils.ClientPosUtil;
...
public class TeleportCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(argument("pos", ClientPosUtil.pos()).executes(ctx -> {
            Vec3d pos = ClientPosUtil.getPos(ctx, "pos");
            mc.player.updatePosition(pos.getX(), pos.getY(), pos.getZ());
            return SINGLE_SUCCESS;
        }));

        builder.then(argument("pos", ClientPosUtil.pos()).then(argument("yaw", FloatArgumentType.floatArg()).then(argument("pitch",FloatArgumentType.floatArg()).executes(ctx -> {
            Vec3d pos = ClientPosUtil.getPos(ctx, "pos");
