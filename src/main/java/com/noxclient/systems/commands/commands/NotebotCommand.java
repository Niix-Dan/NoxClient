...
import com.noxclient.systems.modules.misc.Notebot;
...
public class NotebotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("play").then(argument("name", StringArgumentType.greedyString()).executes(ctx -> {
            ...
            Path path = NoxClient.FOLDER.toPath().resolve(String.format("notebot/%s.txt", name));
            if (!path.toFile().exists()) {
                path = NoxClient.FOLDER.toPath().resolve(String.format("notebot/%s.nbs", name));
            }
