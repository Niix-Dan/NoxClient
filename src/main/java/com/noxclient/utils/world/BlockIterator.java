...
public class BlockIterator {
    ...
    public static void init() {
        NoxClient.EVENT_BUS.subscribe(BlockIterator.class);
    }
    ...
    private static void onTick(TickEvent.Pre event) {
        ...
        int px = NoxClient.mc.player.getBlockX();
        int py = NoxClient.mc.player.getBlockY();
        int pz = NoxClient.mc.player.getBlockZ();

        for (int x = px - hRadius; x <= px + hRadius; x++) {
            for (int z = pz - hRadius; z <= pz + hRadius; z++) {
                for (int y = Math.max(NoxClient.mc.world.getBottomY(), py - vRadius); y <= py + vRadius; y++) {
                    if (y > NoxClient.mc.world.getTopY()) break;

                    blockPos.set(x, y, z);
                    BlockState blockState = NoxClient.mc.world.getBlockState(blockPos);
