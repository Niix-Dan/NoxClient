...
public class MeteorToast implements Toast {
    ...
    public Visibility draw(MatrixStack matrices, ToastManager toastManager, long currentTime) {
        ...
        if (text != null) {
            NoxClient.mc.textRenderer.draw(matrices, text, x, 18, TEXT_COLOR);
            titleY = 7;
        }

        NoxClient.mc.textRenderer.draw(matrices, title, x, titleY, TITLE_COLOR);

        if (icon != null) NoxClient.mc.getItemRenderer().renderInGui(icon, 8, 8);

        if (!playedSound) {
            NoxClient.mc.getSoundManager().play(getSound());
            playedSound = true;
        }
