...
public class InvUtils {
    ...
    public static boolean switchtoitem(FindItemResult item, boolean quickmove, boolean force, Item iteminmessage) {
        ...
        if (item.found()) {
            if (item.isHotbar()) {
                ...
                if (empty.found() && empty.isHotbar()) {
                    ...
                    ChatUtils.info("no space in hotbar");
                    return false;
                }
            }
        }else {
            ChatUtils.info("no " + (iteminmessage != null ? iteminmessage : "required item") + " found");
            return false;
        }
    }


    public static boolean switchtoitem(Item item, boolean quickmove, boolean force) {
        if (mc.player != null && mc.player.getMainHandStack().getItem() == item) return true;
        return switchtoitem(InvUtils.find(item), quickmove, force, item);
    }
