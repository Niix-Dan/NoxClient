...
import com.noxclient.mixin.CreativeInventoryScreenAccessor;
import com.noxclient.mixin.HorseScreenHandlerAccessor;
...
public class SlotUtils {
    ...
    public static int indexToId(int i) {
        if (NoxClient.mc.player == null) return -1;
        ScreenHandler handler = NoxClient.mc.player.currentScreenHandler;
    ...
    private static int creativeInventory(int i) {
        if (!(NoxClient.mc.currentScreen instanceof CreativeInventoryScreen) || CreativeInventoryScreenAccessor.getSelectedTab() != ItemGroup.INVENTORY.getIndex()) return -1;
