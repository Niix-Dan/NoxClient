...
import com.noxclient.addons.GithubRepo;
import com.noxclient.addons.MeteorAddon;
import com.noxclient.gui.GuiThemes;
import com.noxclient.gui.tabs.screens.CommitsScreen;
...
public class TitleScreenCredits {
    ...
    public static void render(MatrixStack matrices) {
        ...
        int y = 3;
        for (Credit credit : credits) {
            int x = NoxClient.mc.currentScreen.width - 3 - credit.width;

            synchronized (credit.sections) {
                for (Section section : credit.sections) {
                    NoxClient.mc.textRenderer.drawWithShadow(matrices, section.text, x, y, section.color);
                    x += section.width;
                }
            }

            y += NoxClient.mc.textRenderer.fontHeight + 2;
        }
    }
    ...
    public static boolean onClicked(double mouseX, double mouseY) {
        ...
        for (Credit credit : credits) {
            ...
            if (mouseX >= x && mouseX <= x + credit.width && mouseY >= y && mouseY <= y + NoxClient.mc.textRenderer.fontHeight + 2) {
                if (credit.addon.getRepo() != null && credit.addon.getCommit() != null) {
                    NoxClient.mc.setScreen(new CommitsScreen(GuiThemes.get(), credit.addon));
                    return true;
                }
            }
    ...
    private static class Section {
        ...
        public Section(String text, int color) {
            this.text = text;
            this.color = color;
            this.width = NoxClient.mc.textRenderer.getWidth(text);
        }
    }
