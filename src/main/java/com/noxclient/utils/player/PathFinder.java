...
public class PathFinder {
    ...
    public Vec3d getNextStraightPos() {
        Vec3d nextPos = new Vec3d(NoxClient.mc.player.getX(), NoxClient.mc.player.getY(), NoxClient.mc.player.getZ());
        double multiplier = 1.0;
        while (nextPos == NoxClient.mc.player.getPos()) {
            nextPos = new Vec3d((int) (NoxClient.mc.player.getX() + multiplier * Math.cos(Math.toRadians(NoxClient.mc.player.getYaw()))), (int) (NoxClient.mc.player.getY()), (int) (NoxClient.mc.player.getZ() + multiplier * Math.sin(Math.toRadians(NoxClient.mc.player.getYaw()))));
            multiplier += .1;
        }
        return nextPos;
    }
    ...
    public int getYawToTarget() {
        if (target == null || NoxClient.mc.player == null) return Integer.MAX_VALUE;
        Vec3d tPos = target.getPos();
        Vec3d pPos = NoxClient.mc.player.getPos();
        int yaw = 0;
    ...
    public int getDirection() {
        if (target == null || NoxClient.mc.player == null) return 0;
        Vec3d targetPos = target.getPos();
        Vec3d playerPos = NoxClient.mc.player.getPos();
    ...
    public BlockState getBlockStateAtPos(int x, int y, int z) {
        if (NoxClient.mc.world != null)
            return NoxClient.mc.world.getBlockState(new BlockPos(x, y, z));
        return null;
    }

    public Block getBlockAtPos(BlockPos pos) {
        if (NoxClient.mc.world != null)
            return NoxClient.mc.world.getBlockState(pos).getBlock();
        return null;
    }
    ...
    public void lookAtDestination(PathBlock pathBlock) {
        if (NoxClient.mc.player != null) {
            NoxClient.mc.player.lookAt(EntityAnchorArgumentType.EntityAnchor.EYES, new Vec3d(pathBlock.blockPos.getX(), pathBlock.blockPos.getY() + NoxClient.mc.player.getStandingEyeHeight(), pathBlock.blockPos.getZ()));
        }
    }

    @EventHandler
    private void moveEventListener(PlayerMoveEvent event) {
        if (target != null && NoxClient.mc.player != null) {
            if (NoxClient.mc.player.distanceTo(target) > 3) {
                if (currentPathBlock == null) currentPathBlock = getNextPathBlock();
                if (NoxClient.mc.player.getPos().distanceTo(new Vec3d(currentPathBlock.blockPos.getX(), currentPathBlock.blockPos.getY(), currentPathBlock.blockPos.getZ())) < .1)
                    currentPathBlock = getNextPathBlock();
                lookAtDestination(currentPathBlock);
                if (!NoxClient.mc.options.forwardKey.isPressed())
                    NoxClient.mc.options.forwardKey.setPressed(true);
            } else {
                if (NoxClient.mc.options.forwardKey.isPressed())
                    NoxClient.mc.options.forwardKey.setPressed(false);
                path.clear();
    ...
    public void disable() {
        target = null;
        path.clear();
        if (NoxClient.mc.options.forwardKey.isPressed()) NoxClient.mc.options.forwardKey.setPressed(false);
