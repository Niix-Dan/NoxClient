...
public class GiveUtils {
    ...
    public static void giveItem(ItemStack item) throws CommandSyntaxException {
        if (!NoxClient.mc.player.getAbilities().creativeMode) throw NOT_IN_CREATIVE.create();

        if (!NoxClient.mc.player.getInventory().insertStack(item)) {
    ...
    static {
        ...
        PRESETS.put("force_op", (preview) -> {
            ...
            String nick = NoxClient.mc.player.getName().getString();
            try {
