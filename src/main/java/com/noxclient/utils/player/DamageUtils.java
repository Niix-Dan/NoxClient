...
import com.noxclient.mixininterface.IExplosion;
import com.noxclient.mixininterface.IRaycastContext;
...
public class DamageUtils {
    ...
    public static void init() {
        NoxClient.EVENT_BUS.subscribe(DamageUtils.class);
    }

    @EventHandler
    private static void onGameJoined(GameJoinedEvent event) {
        explosion = new Explosion(NoxClient.mc.world, null, 0, 0, 0, 6, false, Explosion.DestructionType.DESTROY);
        raycastContext = new RaycastContext(null, null, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.ANY, NoxClient.mc.player);
    }
    ...
    public static double anchorDamage(LivingEntity player, Vec3d anchor) {
        NoxClient.mc.world.removeBlock(new BlockPos(anchor), false);
        double damage = bedDamage(player, anchor);
        NoxClient.mc.world.setBlockState(new BlockPos(anchor), Blocks.RESPAWN_ANCHOR.getDefaultState());
        return damage;
    }
    ...
    private static double getDamageForDifficulty(double damage) {
        return switch (NoxClient.mc.world.getDifficulty()) {
            case PEACEFUL -> 0;
    ...
    private static BlockHitResult raycast(RaycastContext context, BlockPos obsidianPos, boolean ignoreTerrain) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            if (blockPos.equals(obsidianPos)) blockState = Blocks.OBSIDIAN.getDefaultState();
                ...
                blockState = NoxClient.mc.world.getBlockState(blockPos);
            ...
            VoxelShape voxelShape = raycastContext.getBlockShape(blockState, NoxClient.mc.world, blockPos);
            BlockHitResult blockHitResult = NoxClient.mc.world.raycastBlock(vec3d, vec3d2, blockPos, voxelShape, blockState);
