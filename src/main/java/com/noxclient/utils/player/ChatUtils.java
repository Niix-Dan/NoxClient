...
import com.noxclient.mixininterface.IChatHud;
...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Text.literal("")
            ...
            .append("[")
            .append(Text.literal("Nox").setStyle(Style.EMPTY.withColor(new TextColor(NoxClient.ADDON.color.getPacked()))))
            .append("] ");
    }
    ...
    public static void sendPlayerMsg(String message) {
        NoxClient.mc.inGameHud.getChatHud().addToMessageHistory(message);

        if (message.startsWith("/")) NoxClient.mc.player.sendCommand(message.substring(1), null);
        else NoxClient.mc.player.sendChatMessage(message, null);
    }
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        if (NoxClient.mc.world == null) return;
        ...
        ((IChatHud) NoxClient.mc.inGameHud.getChatHud()).add(message, id);
    }
