...
import com.noxclient.mixin.CrossbowItemAccessor;
import com.noxclient.mixin.ProjectileInGroundAccessor;
import com.noxclient.mixininterface.IVec3d;
import com.noxclient.utils.Utils;
import com.noxclient.utils.misc.MissHitResult;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double simulated, boolean accurate, double tickDelta) {
        ...
        if (item instanceof BowItem) {
            double charge = BowItem.getPullProgress(NoxClient.mc.player.getItemUseTime());
    ...
    public HitResult tick() {
        ...
        if (pos.y < NoxClient.mc.world.getBottomY()) return MissHitResult.INSTANCE;

        // Check if chunk is loaded
        int chunkX = (int) (pos.x / 16);
        int chunkZ = (int) (pos.z / 16);
        if (!NoxClient.mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) return MissHitResult.INSTANCE;
    ...
    private boolean isTouchingWater() {
        ...
        FluidState fluidState = NoxClient.mc.world.getFluidState(blockPos);
    ...
    private HitResult getCollision() {
        ...
        HitResult hitResult = NoxClient.mc.world.raycast(new RaycastContext(vec3d3, pos3d, RaycastContext.ShapeType.COLLIDER, waterDrag == 0 ? RaycastContext.FluidHandling.ANY : RaycastContext.FluidHandling.NONE, NoxClient.mc.player));
        if (hitResult.getType() != HitResult.Type.MISS) {
            vec3d3 = hitResult.getPos();
        }

        HitResult hitResult2 = ProjectileUtil.getEntityCollision(NoxClient.mc.world, NoxClient.mc.player, vec3d3, pos3d, new Box(pos.x, pos.y, pos.z, pos.x, pos.y, pos.z).stretch(NoxClient.mc.player.getVelocity()).expand(1.0D), entity -> !entity.isSpectator() && entity.isAlive() && entity.canHit());
        if (hitResult2 != null) {
