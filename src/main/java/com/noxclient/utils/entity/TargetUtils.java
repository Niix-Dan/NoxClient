...
public class TargetUtils {
    ...
    public static PlayerEntity getPlayerTarget(double range, SortPriority priority) {
        ...
        return (PlayerEntity) get(entity -> {
            if (!(entity instanceof PlayerEntity) || entity == NoxClient.mc.player) return false;
            if (((PlayerEntity) entity).isDead() || ((PlayerEntity) entity).getHealth() <= 0) return false;
            if (NoxClient.mc.player.distanceTo(entity) > range) return false;
    ...
    public static boolean isBadTarget(PlayerEntity target, double range) {
        ...
        return NoxClient.mc.player.distanceTo(target) > range || !target.isAlive() || target.isDead() || target.getHealth() <= 0;
    }

    private static int sort(Entity e1, Entity e2, SortPriority priority) {
        return switch (priority) {
            case LowestDistance -> Double.compare(e1.distanceTo(NoxClient.mc.player), e2.distanceTo(NoxClient.mc.player));
            case HighestDistance -> invertSort(Double.compare(e1.distanceTo(NoxClient.mc.player), e2.distanceTo(NoxClient.mc.player)));
    ...
    private static int sortAngle(Entity e1, Entity e2) {
        ...
        double e1yaw = Math.abs(Rotations.getYaw(e1) - NoxClient.mc.player.getYaw());
        double e2yaw = Math.abs(Rotations.getYaw(e2) - NoxClient.mc.player.getYaw());

        double e1pitch = Math.abs(Rotations.getPitch(e1) - NoxClient.mc.player.getPitch());
        double e2pitch = Math.abs(Rotations.getPitch(e2) - NoxClient.mc.player.getPitch());
