...
package com.noxclient.utils.tooltip;
...
public class MapTooltipComponent implements TooltipComponent, MeteorTooltipData {
    ...
    public void drawItems(TextRenderer textRenderer, int x, int y, MatrixStack matrices, ItemRenderer itemRenderer, int z) {
        ...
        // Contents
        VertexConsumerProvider.Immediate consumer = NoxClient.mc.getBufferBuilders().getEntityVertexConsumers();
        MapState mapState = FilledMapItem.getMapState(this.mapId, NoxClient.mc.world);
        if (mapState == null) return;
        matrices.push();
        matrices.translate(x, y, z);
        matrices.scale((float) scale, (float) scale, 0);
        matrices.translate(8, 8, 0);
        NoxClient.mc.gameRenderer.getMapRenderer().draw(matrices, consumer, this.mapId, mapState, false, 0xF000F0);
        consumer.draw();
        matrices.pop();
    }
}
