...
import com.noxclient.utils.render.prompts.OkPrompt;
...
public class NbtUtils {
    ...
    public static boolean toClipboard(String name, NbtCompound nbtCompound) {
        String preClipboard = NoxClient.mc.keyboard.getClipboard();
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            NbtIo.writeCompressed(nbtCompound, byteArrayOutputStream);

            NoxClient.mc.keyboard.setClipboard(Base64.getEncoder().encodeToString(byteArrayOutputStream.toByteArray()));
            return true;
        } catch (Exception e) {
            NoxClient.LOG.error(String.format("Error copying %s NBT to clipboard!", name));
            ...
            NoxClient.mc.keyboard.setClipboard(preClipboard);
            return false;
        }
    }
    ...
    public static NbtCompound fromClipboard(NbtCompound schema) {
        try {
            byte[] data = Base64.getDecoder().decode(NoxClient.mc.keyboard.getClipboard().trim());
            ...
            NoxClient.LOG.error("Invalid NBT data pasted!");
