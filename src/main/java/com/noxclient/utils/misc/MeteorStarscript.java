...
import com.noxclient.utils.world.TickRate;
...
public class MeteorStarscript {
    ...
    public static void init() {
        ...
        // General
        ss.set("version", NoxClient.VERSION != null ? (NoxClient.DEV_BUILD.isEmpty() ? NoxClient.VERSION.toString() : NoxClient.VERSION + " " + NoxClient.DEV_BUILD) : "");
        ...
        ss.set("camera", new ValueMap()
            .set("pos", new ValueMap()
                ...
                .set("x", () -> Value.number(NoxClient.mc.gameRenderer.getCamera().getPos().x))
                .set("y", () -> Value.number(NoxClient.mc.gameRenderer.getCamera().getPos().y))
                .set("z", () -> Value.number(NoxClient.mc.gameRenderer.getCamera().getPos().z))
            )
        ...
        ss.set("player", new ValueMap()
            .set("_toString", () -> Value.string(NoxClient.mc.getSession().getUsername()))
            .set("health", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.getHealth() : 0))
            .set("hunger", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.getHungerManager().getFoodLevel() : 0))
            .set("speed", () -> Value.number(Utils.getPlayerSpeed()))

            .set("breaking_progress", () -> Value.number(NoxClient.mc.interactionManager != null ? ((ClientPlayerInteractionManagerAccessor) NoxClient.mc.interactionManager).getBreakingProgress() : 0))
            ...
            .set("pos", new ValueMap()
                ...
                .set("x", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.getX() : 0))
                .set("y", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.getY() : 0))
                .set("z", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.getZ() : 0))
            )
            .set("opposite_dim_pos", new ValueMap()
            ...
            .set("hand", () -> NoxClient.mc.player != null ? wrap(NoxClient.mc.player.getMainHandStack()) : Value.null_())
            .set("offhand", () -> NoxClient.mc.player != null ? wrap(NoxClient.mc.player.getOffHandStack()) : Value.null_())
            ...
            .set("xp", new ValueMap()
                .set("level", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.experienceLevel : 0))
                .set("progress", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.experienceProgress : 0))
                .set("total", () -> Value.number(NoxClient.mc.player != null ? NoxClient.mc.player.totalExperience : 0))
            )
        ...
        ss.set("server", new ValueMap()
            ...
            .set("player_count", () -> Value.number(NoxClient.mc.getNetworkHandler() != null ? NoxClient.mc.getNetworkHandler().getPlayerList().size() : 0))
            .set("difficulty", () -> Value.string(NoxClient.mc.world != null ? NoxClient.mc.world.getDifficulty().getName() : ""))
        );
    }
    ...
    private static Value hasPotionEffect(Starscript ss, int argCount) {
        ...
        if (NoxClient.mc.player == null) return Value.bool(false);
        ...
        StatusEffectInstance effectInstance = NoxClient.mc.player.getStatusEffect(effect);
    ...
    private static Value getPotionEffect(Starscript ss, int argCount) {
        ...
        if (NoxClient.mc.player == null) return Value.null_();
    ...
    private static Value getStat(Starscript ss, int argCount) {
        ...
        if (NoxClient.mc.player == null) return Value.number(0);

        long time = System.currentTimeMillis();
        if ((time - lastRequestedStatsTime) / 1000.0 >= 1 && NoxClient.mc.getNetworkHandler() != null) {
            NoxClient.mc.getNetworkHandler().sendPacket(new ClientStatusC2SPacket(ClientStatusC2SPacket.Mode.REQUEST_STATS));
        ...
        return Value.number(stat != null ? NoxClient.mc.player.getStatHandler().getStat(stat) : 0);
    }
    ...
    private static Value getItem(Starscript ss, int argCount) {
        ...
        return NoxClient.mc.player != null ? wrap(NoxClient.mc.player.getInventory().getStack(i)) : Value.null_();
    }
    ...
    private static Value countItems(Starscript ss, int argCount) {
        ...
        if (item == Items.AIR || NoxClient.mc.player == null) return Value.number(0);

        int count = 0;
        for (int i = 0; i < NoxClient.mc.player.getInventory().size(); i++) {
            ItemStack itemStack = NoxClient.mc.player.getInventory().getStack(i);
    ...
    private static Value oppositeX(boolean camera) {
        double x = camera ? NoxClient.mc.gameRenderer.getCamera().getPos().x : (NoxClient.mc.player != null ? NoxClient.mc.player.getX() : 0);
    ...
    private static Value oppositeZ(boolean camera) {
        double z = camera ? NoxClient.mc.gameRenderer.getCamera().getPos().z : (NoxClient.mc.player != null ? NoxClient.mc.player.getZ() : 0);
        Dimension dimension = PlayerUtils.getDimension();
    ...
    private static Value yaw(boolean camera) {
        float yaw;
        if (camera) yaw = NoxClient.mc.gameRenderer.getCamera().getYaw();
        else yaw = NoxClient.mc.player != null ? NoxClient.mc.player.getYaw() : 0;
        yaw %= 360;
    ...
    private static Value pitch(boolean camera) {
        float pitch;
        if (camera) pitch = NoxClient.mc.gameRenderer.getCamera().getPitch();
        else pitch = NoxClient.mc.player != null ? NoxClient.mc.player.getPitch() : 0;
        pitch %= 360;
    ...
    private static Value biome() {
        if (NoxClient.mc.player == null || NoxClient.mc.world == null) return Value.string("");

        BP.set(NoxClient.mc.player.getX(), NoxClient.mc.player.getY(), NoxClient.mc.player.getZ());
        Identifier id = NoxClient.mc.world.getRegistryManager().get(Registry.BIOME_KEY).getId(NoxClient.mc.world.getBiome(BP).value());
    ...
    private static Value handOrOffhand() {
        ...
        if (itemStack.isEmpty()) itemStack = NoxClient.mc.player.getOffHandStack();

        return itemStack != null ? wrap(itemStack) : Value.null_();
    }

    private static Value ping() {
        if (NoxClient.mc.getNetworkHandler() == null || NoxClient.mc.player == null) return Value.number(0);

        PlayerListEntry playerListEntry = NoxClient.mc.getNetworkHandler().getPlayerListEntry(NoxClient.mc.player.getUuid());
    ...
    private static Value baritoneDistanceToGoal() {
        ...
        return Value.number((goal != null && NoxClient.mc.player != null) ? goal.heuristic(NoxClient.mc.player.getBlockPos()) : 0);
    }

    private static Value posString(boolean opposite, boolean camera) {
        Vec3d pos;
        if (camera) pos = NoxClient.mc.gameRenderer.getCamera().getPos();
        else pos = NoxClient.mc.player != null ? NoxClient.mc.player.getPos() : Vec3d.ZERO;
    ...
    private static Value crosshairType() {
        if (NoxClient.mc.crosshairTarget == null) return Value.string("miss");

        return Value.string(switch (NoxClient.mc.crosshairTarget.getType()) {
            case MISS -> "miss";
            case BLOCK -> "block";
            case ENTITY -> "entity";
        });
    ...
    private static Value crosshairValue() {
        if (NoxClient.mc.world == null || NoxClient.mc.crosshairTarget == null) return Value.null_();

        if (NoxClient.mc.crosshairTarget.getType() == HitResult.Type.MISS) return Value.string("");
        if (NoxClient.mc.crosshairTarget instanceof BlockHitResult hit) return wrap(hit.getBlockPos(), NoxClient.mc.world.getBlockState(hit.getBlockPos()));
        return wrap(((EntityHitResult) NoxClient.mc.crosshairTarget).getEntity());
    }
